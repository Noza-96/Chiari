    ...
    elseif strcmp(reference_location, 'fromsag') == 1
        ...
        if exist([cas.dirgeo, '/geomdata.mat']) == 0
            ...
            for idat = 1:Ndat
                ...
                text(1, locz{idat}, locations{idat}, 'Color', 'r', 'FontSize', 8, 'FontWeight', 'bold')
                hold on
                if isfield(geo, strcat('z', locations{idat})) == 1
                    plot(0, geo.(strcat('z', locations{idat})),  'k^', 'MarkerFaceColor', 'k')
                    hold on
                    text(1, geo.(strcat('z', locations{idat})), locations{idat}, 'Color', 'k', 'FontSize', 8, 'FontWeight', 'bold')
                    hold on
            ...
            % The obtain the real location of the marked position (in cm):
            loczmark = geo.(strcat('z', reference_location));

            % The look up the marked position in the data:
            indmark = find(strcmp(locations, reference_location));

            indmark = indmark(1);
            
            % Now we shift the location data with the correct amount:
            shift = loczmark - locz{indmark}
            for idat = 1:Ndat
                locz{idat} = locz{idat} + shift;
            end

            % Plot shifted locations:
            figure(882)
            subplot(1, 2, 2)
            for idat = 1:Ndat
                plot(0, locz{idat}, 'bo', 'MarkerFaceColor', 'b')
            ...
            % Finally, we compute the along-spine distance to the FM:
            for idat = 1:Ndat
                locd{idat} = geo.convert_z2d(locz{idat});
            end
