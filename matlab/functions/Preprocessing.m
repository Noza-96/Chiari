% 1. Preprocessing after running PC-MRI
clear; close all; clc;
addpath('Functions/');
addpath('Functions/Others/')

% Choose subject
subject = "s101_b"; 

% c: geometry boundded by 2 pcMRI planes. 
% c0/c1 for zero pressure top and bottom flow rate/velocity
% c2 for two inlet velocities; continuity: normal velocity tonsils 
% c3 for two inlet velocities; continuity: multiple regions

case_name = {"cnl3_v1", "cl3_v1"};      % Array with the kind of simulations to do
mesh_size = [0.0002];    % Array with the different mesh sizes to be simulated

ts_cycle = 100;     % number of time steps per cycle
iterations_ts = 20; % iterations per time step
cycles = 3;         % cyles to be computed
n_cores = 14;       % number of processors simulation

[cas, dat_PC] = check_subject_initialization (subject, ts_cycle, 1);

[DNS_cases, nerve_sim, lig_sim, nerve_lig_sim, zones_sim] = create_DNS_cases (case_name, mesh_size, cas, cycles, iterations_ts, ts_cycle);

% run fluent-meshing to create meshes and corresponding .cas files
cases_ready = GUI_create_mesh(cas, mesh_size, case_name);

answer = questdlg('Run ANSYS simulation?', 'Confirmation', 'Yes', 'No', 'No');
if strcmp(answer, 'Yes')
    disp('Running ANSYS simulation...');   
    run_ANSYS_simulations (cas, dat_PC, DNS_cases, n_cores, 1)
else
end


%% Auxiliary functions 

function [cas, dat_PC] = check_subject_initialization(subject, ts_cycle, repeat_initialization)
    if nargin < 3
        repeat_initialization = 0;
    end

    % load data
    mri_data_path = fullfile("../../../computations", "pc-mri", subject, "mat", "04-registration.mat");
    load(mri_data_path, 'cas', 'dat_PC');
    
    % auxiliary file used to see if ANSYS data is up to date
    file_1 = fullfile(cas.diransys_in, "flow-rates", "Q_bottom.txt");
    folder_2 = fullfile(cas.diransys_in, "profiles", "ts_"+ts_cycle); 

    % Create ansys inputs if needed or if forced by repeat_initialization
    d1 = dir(mri_data_path);
    d2 = dir(file_1);
    

    if repeat_initialization || exist(file_1, 'file') == 0 || exist(folder_2, 'dir') == 0 || ...
        datetime(d1.datenum, 'ConvertFrom', 'datenum') > datetime(d2.datenum, 'ConvertFrom', 'datenum')
        disp('ansys inputs need to be created\updated, creating files...')
        create_ansys_inputs(dat_PC, cas, ts_cycle);
    else 
        disp('ansys inputs are up to date ...')
    end
end