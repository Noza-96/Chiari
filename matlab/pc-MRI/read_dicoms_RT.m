function dat = read_dicoms_RT(cas, resettimevector)

    Ndat = length(cas.folders_RT);

    % Get data for each case and store in cell structures were first dimension is case number:

    for idat = 1:Ndat
        
        disp([cas.dirdcm, '/', cas.folders_RT_P{idat}]);
        
        dicomlist = dir(fullfile([cas.dirdcm, '/', cas.folders_RT_P{idat}], '*.dcm'));
        ...
        for jj = 1:numim
            ...
            fname = fullfile([cas.dirdcm, '/', cas.folders_RT_P{idat}], dicomlist(jj).name);
        ...
        if isempty(cas.folders_RT_MAG) == 1
            ...
            dicomlist = dir(fullfile([cas.dirdcm, '/', cas.folders_RT_MAG{idat}], '*.dcm'));

            numim = numel(dicomlist);

            fname_showorient{idat} = fullfile([cas.dirdcm, '/', cas.folders_RT_MAG{idat}], dicomlist(1).name);

            for jj = 1:numim

                fname = fullfile([cas.dirdcm, '/', cas.folders_RT_MAG{idat}], dicomlist(jj).name);
        ...
        dicomlist = dir(fullfile([cas.dirdcm, '/', cas.folders_RT_{idat}], '*.dcm'));

        numim = numel(dicomlist);

        for jj = 1:numim

            fname = fullfile([cas.dirdcm, '/', cas.folders_RT_{idat}], dicomlist(jj).name);
    ...
    saveas(fig, [cas.dirfig, '/orientation_of_planes_RT.fig'])
