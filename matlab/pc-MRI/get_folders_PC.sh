...
sed_inplace "s|_P.*||" ./aux_PC/folders.txt
sed_inplace  "s|$folder_path/||" ./aux_PC/folders.txt
sort_inplace ./aux_PC/folders.txt
...
sed_inplace "s|$folder_path/||" ./aux_PC/folders_.txt
sort_inplace ./aux_PC/folders_.txt

# 3) FOLDERS (MAG) -> folders_MAG.txt
find "$folder_path" -maxdepth 4 -ipath "*/*PC/*" -and \( -name "*_MAG_*" -o -name "*_MAG" \) > ./aux_PC/folders_MAG.txt
sed_inplace "s|$folder_path/||" ./aux_PC/folders_MAG.txt
sort_inplace ./aux_PC/folders_MAG.txt

# 4) Explicit P list -> folders_P.txt
find "$folder_path" -maxdepth 4 -ipath "*/*PC/*" -and \( -name "*_P_*" -o -name "*_P" \) > ./aux_PC/folders_P.txt
sed_inplace "s|$folder_path/||" ./aux_PC/folders_P.txt
sort_inplace ./aux_PC/folders_P.txt